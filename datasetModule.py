...
class GestureDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str=None):
        ...
        self.train_dataset, dataset = random_split(
                full_dataset,
                [int(0.7 * len(full_dataset)), len(full_dataset) - int(0.7 * len(full_dataset))], 
                generator=torch.Generator().manual_seed(42)
            )
        self.val_dataset, self.test_dataset = random_split(
                dataset,
                [int(0.5 * len(dataset)), len(dataset) - int(0.5 * len(dataset))], 
                generator=torch.Generator().manual_seed(42)
        )
    ...
    def test_dataloader(self):
         return DataLoader(self.test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers, collate_fn=video_collate_fn, persistent_workers=True, pin_memory=False)
